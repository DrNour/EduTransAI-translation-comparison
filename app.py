...
# --- Upload CSV with source and translations ---
uploaded_file = st.file_uploader(
    "Upload your translations CSV (columns: Source, Translation1, Translation2, ...)", 
    type=["csv"]
)
    ...
    if len(translation_cols) >= 2:
        ...
        for idx, row in df.iterrows():
            ...
            try:
                ...
                st.error(f"Embedding API failed at row {idx}: {e}")
                embeddings = [np.zeros(384) for _ in trans_texts]  # fallback
            ...
            # --- Simple error highlighting / diff ---
            diffs = []
