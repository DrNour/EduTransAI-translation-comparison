...
# - Examples & Issue Mining with CSV/DOCX exports
...
def semantic_accuracy_score(
    ...
    # Stabilize by fluency/style floors
...
# ---------------------------
# Human-readable export columns (collision-safe)
# ---------------------------
...
def humanize_export_columns(cols) -> list:
    ...
    return out
# ===========================
# Example mining + exports (helpers)
# ===========================
...
def _pick_cols(res_df, base, suffixes):
    return {s: f"{base}_{s}" for s in suffixes if _col_exists(res_df, f"{base}_{s}")}
...
def _row_pack(i, base, res_df, df, source_col, mode):
    rec = {"Row": i, "Student": base}
    cols_need = _pick_cols(res_df, base, ["Accuracy","Semantic","Cosine","BERTScoreF1","Lexical","Fluency","Style","ErrorsNorm"])
    for k, c in cols_need.items():
        rec[k] = _safe_num(res_df.at[i, c]) if k not in ("ErrorsNorm",) else res_df.at[i, c]
    rec["Source"] = (str(df.at[i, source_col]) if (mode == "Reference-based" and source_col) else "")
    rec["Translation"] = str(df.at[i, base]) if base in df.columns else ""
    return rec
...
def gather_issue_examples(
    ...
    *,
    ...
    """
    Returns a dict of DataFrames keyed by issue label plus a 'Combined' one.
    thresholds keys: LexLow, AccLow, FluLow, StyleLow
    """
    thresholds = thresholds or {}
    LexLow = thresholds.get("LexLow", 0.15)
    AccLow = thresholds.get("AccLow", 0.50)
    FluLow = thresholds.get("FluLow", 3.0)
    StyleLow = thresholds.get("StyleLow", 3.0)
    ...
    for i in range(len(res_df)):
        for base in translation_cols:
            ...
            acc = res_df.get(f"{base}_Accuracy")
            lex = res_df.get(f"{base}_Lexical")
            flu = res_df.get(f"{base}_Fluency")
            sty = res_df.get(f"{base}_Style")
            cos = res_df.get(f"{base}_Cosine")
            bs  = res_df.get(f"{base}_BERTScoreF1")

            sem_v = _safe_num(sem[i]) if isinstance(sem, pd.Series) and i < len(sem) else np.nan
            acc_v = _safe_num(acc[i]) if isinstance(acc, pd.Series) and i < len(acc) else np.nan
            lex_v = _safe_num(lex[i]) if isinstance(lex, pd.Series) and i < len(lex) else np.nan
            flu_v = _safe_num(flu[i]) if isinstance(flu, pd.Series) and i < len(flu) else np.nan
            sty_v = _safe_num(sty[i]) if isinstance(sty, pd.Series) and i < len(sty) else np.nan
            cos_v = _safe_num(cos[i]) if isinstance(cos, pd.Series) and i < len(cos) else np.nan
            bs_v  = _safe_num(bs[i])  if isinstance(bs,  pd.Series) and i < len(bs)  else np.nan

            rec = _row_pack(i, base, res_df, df_orig, source_col, mode)

            if pd.notna(acc_v) and acc_v < AccLow:
                rec["SortScore"] = acc_v
                buckets["Low Accuracy / Semantic"].append(rec)

            if pd.notna(lex_v) and lex_v < LexLow:
                rec2 = dict(rec); rec2["SortScore"] = lex_v
                buckets["Low Lexical Overlap"].append(rec2)

            if pd.notna(flu_v) and flu_v < FluLow:
                rec3 = dict(rec); rec3["SortScore"] = flu_v
                buckets["Low Fluency"].append(rec3)

            if pd.notna(sty_v) and sty_v < StyleLow:
                rec4 = dict(rec); rec4["SortScore"] = sty_v
                buckets["Low Style"].append(rec4)

            # Paraphrase vs Drift (prefer semantic over accuracy)
            ...
            if pd.notna(sem_signal) and pd.notna(lex_v):
                if sem_signal >= paraphrase_sem_hi and lex_v < low_lexical_for_paraphrase:
                    rec5 = dict(rec); rec5["SortScore"] = -sem_signal  # strongest first
                    buckets["Paraphrase (Sem↑ Lex↓)"].append(rec5)
                if sem_signal < drift_sem_lo:
                    rec6 = dict(rec); rec6["SortScore"] = sem_signal
                    buckets["Meaning Drift (Sem↓)"].append(rec6)

            if pd.notna(cos_v) and pd.notna(bs_v) and abs(cos_v - bs_v) > consistency_tolerance:
                rec7 = dict(rec); rec7["SortScore"] = -abs(cos_v - bs_v)
    ...
    for k, rows in buckets.items():
        ...
        out[k] = dfk.head(top_n).drop(columns=["SortScore"], errors="ignore")

    out["Combined"] = pd.concat([v.assign(Issue=k) for k, v in out.items() if not v.empty], axis=0) if any([not v.empty for v in out.values()]) else pd.DataFrame()
    return out

def examples_to_docx(ex_df: pd.DataFrame, title: str = "Examples") -> bytes:
    """Create a simple DOCX with Source / Translation and key metrics."""
    from docx import Document
    doc = Document()
    doc.add_heading(title, level=1)
    for _, r in ex_df.iterrows():
        doc.add_paragraph(f"Row {int(r['Row'])} — {str(r.get('Student',''))}")
        if "Issue" in ex_df.columns:
            doc.add_paragraph(f"Issue: {str(r.get('Issue',''))}")
        mline = []
        for k in ["Accuracy","Semantic","Cosine","BERTScoreF1","Lexical","Fluency","Style"]:
            if k in ex_df.columns and pd.notna(r.get(k, np.nan)):
                try:
                    mline.append(f"{k}={float(r.get(k)):.3f}")
                except Exception:
                    pass
        if mline:
            doc.add_paragraph(" | ".join(mline))
        if "Source" in ex_df.columns and str(r.get("Source","")).strip():
            doc.add_paragraph("Source:")
            doc.add_paragraph(str(r.get("Source","")))
        doc.add_paragraph("Translation:")
        doc.add_paragraph(str(r.get("Translation","")))
        doc.add_paragraph("--------------------")
    bio = BytesIO()
    doc.save(bio)
    return bio.getvalue()
