...
uploaded_file = st.file_uploader(
    "Upload CSV, Excel, or Word file (translations with Source, GT, Student, Reference)",
    ...
    try:
        ...
        if translation_cols and st.button("Run Analysis"):
            ...
            for idx, row in df.iterrows():
                row_result = {}
                if source_col:
                    row_result["Source"] = str(row[source_col])
                for t_col in translation_cols:
                    trans_text = str(row[t_col])

                    if mode == "Reference-based":
                        bleu = sentence_bleu([word_tokenize(str(row[source_col]))], word_tokenize(trans_text), smoothing_function=smoothie)
                        fluency, style_score, error_str = enhanced_error_detection(trans_text, reference=str(row[source_col]), embed_cache=embed_cache)
                        ...
                        for other_col in translation_cols:
                            ...
                            fluency, style_score, error_str = enhanced_error_detection(trans_text, other_texts=[other_text], embed_cache=embed_cache)
                        ...
                        fluency, style_score, error_str = enhanced_error_detection(trans_text, embed_cache=embed_cache)
                        row_result[f"{t_col}_Fluency"] = fluency
                        row_result[f"{t_col}_Style"] = style_score
                        row_result[f"{t_col}_Errors"] = error_str
            ...
            for col in res_df.columns:
                if col.endswith("_Fluency"):
                    res_df[col.replace("_Fluency","_Low_Fluency_Flag")] = res_df[col].apply(lambda x:"⚠️" if x<3 else "")
                elif col.endswith("_Style"):
                    res_df[col.replace("_Style","_Low_Style_Flag")] = res_df[col].apply(lambda x:"⚠️" if x<0.5 else "")
                elif col.endswith("_BLEU"):
                    res_df[col.replace("_BLEU","_Low_BLEU_Flag")] = res_df[col].apply(lambda x:"⚠️" if x<0.5 else "")
            st.dataframe(res_df.head(20))
            ...
            st.download_button("Download Full Analysis CSV", csv, "translation_analysis_results.csv", "text/csv")
